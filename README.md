...
В каждом из датацентров будет установлен L7-балансировщик для равномерного распределения нагрузки на ингресс-контроллер Kubernetes с использованием алгоритма round robin. Для этой цели выбран nginx ingress controller, так как он регулярно обновляется и имеет активное сообщество пользователей. Используя эти ингрессы, SSL-трафик будет терминироваться для сокращения нагрузки на установку соединений внутри доверенной сети Kubernetes. Ингресс-контроллеры будут следовать указанным правилам для эффективного распределения нагрузки на сервисы Kubernetes. 

### 4.2 Схема балансировки для межсервисных запросов

В Kubernetes балансировка нагрузки между сервисами осуществляется через два уровня абстракций. Первый уровень - это сервисы Kubernetes, которые используют iptables для реализации L7-балансировки. Второй уровень - балансировка через DNS с помощью headless service, что особенно полезно для микросервисной архитектуры. Таким образом, в Kubernetes присутствует балансировка как на уровне DNS, так и на уровне прикладного слоя.
