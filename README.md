...
> 133000 RPS * 5 Кб = 0.63 Гбит/c
> 11 500 000 000 * 5 кб = 61.44 Гбит/сутки
...
В каждом датацентре будет находиться L7-балансировщик, предоствляемый облачным провайдером (ex. aws, gcs), который будет распределять нагрузку на ingress controller Kubernetes по алгоритму round robin. На ингрессах будет происходить терминации ssl трафика, для уменьшения оверхеда на установления соединения в доверенной сети Kubernetes. Ингресс контроллеры в соотвествии с указанными правилами распределяют нагрузки на сервисы Kubernetes.

### 4.2 Схема балансировки для межсервисных запросов

Балансировка нагрузки межсервисных запросов в Kubernetes реализована с помощью абстракций Kubernetes - сервисов, которые реализованы правилами iptables, то есть является L7 балансировкой. Также в Kubernetes предусмотрена балансировка на уровне dns - headless service, которые разумно применять для микросервисов и stateful set'ов. Соотсвественно часть нагрузки будет балансироваться на уровне dns, а часть на прикладном уровне.

### 4.3 Схема отказоустойчивости
Для обеспечения отказоустойчивости и горизонтального масштабирования в данном случае будут использоваться Kubernetes и Nginx.

Горизонтальное масштабирование будет реализовано с помощью абстракции Kubernetes, известной как Horizontal Pod Autoscaling. Она позволяет автоматически изменять количество экземпляров (подов) приложения в зависимости от настроенных метрик.

Kubernetes также обеспечивает отказоустойчивость путем удаления зависших приложений, которые не отвечают на health check'и. После этого Kubernetes запускает новые экземпляры приложений, убеждается в их корректном запуске и только затем направляет на них нагрузку, что помогает обеспечить непрерывную работу системы.
